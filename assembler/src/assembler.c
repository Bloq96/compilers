...
void splitLine(char *input, int inputLength, char *output[3], int *outputLength)
{
    int el = 0, it2 = 0;
    ...
    while (len != EOF)
    {
        ...
        if (strings[0][lengths[0] - 1] != ':')
        {
            ...
            if (len > 0)
            {
                if (len < 17)
                {
                    fprintf(outputFile, "%d ", encode(strings[0], lengths[0]));
                }
                if (len < 15)
                {
                    ++pc;
                    fprintf(outputFile, "%d ", (lookup(&lut, strings[1], lengths[1]) - pc));
                }
            }
        }
        else
        {
            ...
            if (len > 0)
            {
                if (len < 17)
                {
                    fprintf(outputFile, "%d ", encode(strings[1], lengths[1]));
                }
                if (len < 15)
                {
                    ++pc;
                    fprintf(outputFile, "%d ", (lookup(&lut, strings[2], lengths[2]) - pc));
                }
            }
            else if (len == 0)
            {
